...
describe('User Controller', () => {
  ...
  describe('When a user signs up', () => {
    it(
      'Then it should return the user details and authentication token',
      async () => {
  ...
  describe('When a user signs in', () => {
    it(
      'should return user details and authenticaion token',
      async () => {
        const response = await request(app)
  ...
  describe('When a user signs up with existing user data', () => {
    it(
      'should return an error message containing why the user cannot be registered',
      async () => {
        const response = await request(app)
          .post('/api/v1/users/signup')
          .set('Accept', 'application/x-www-form-urlencoded')
  ...
  describe(
    'When a user signs up with an invalid firstName and lastName',
    () => {
      it(
        'should return `Invalid first name` and `Invalid last name`',
        async () => {
          const response = await request(app)
            .post('/api/v1/users/signup')
            .set('Accept', 'application/x-www.form-urlencoded')
          ...
          expect(JSON.parse(response.error.text).errors[0].msg)
            .to.equal('Invalid first name');
          expect(JSON.parse(response.error.text).errors[1].msg)
            .to.equal('Invalid last name');
        }
      );
    }
  );
  ...
  describe('When a user signs up with an invalid password', () => {
    it(
      'should return error message containing why the password is invalid',
      async () => {
        const response = await request(app)
          .post('/api/v1/users/signup')
          .set('Accept', 'application/x-www.form-urlencoded')
        ...
        expect(JSON.parse(response.error.text).errors[0].msg)
          .to.equal('Paswword must be at least 8 characters');
      }
    );
    ...
    it(
      ...
      async () => {
        ...
        expect(JSON.parse(response.error.text).errors[0].msg)
          .to.equal('Paswword must contain at least one number');
      }
    );
  });

  describe(
    'When a user signs in an invalid email and an empty password',
    () => {
      it(
        'should return `Email is required` and `Password is required`',
        async () => {
          const response = await request(app)
            .post('/api/v1/users/signin')
            .set('Accept', 'application/x-www-form-urlencoded')
